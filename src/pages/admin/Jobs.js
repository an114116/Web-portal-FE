import {
    DataTable,
    Theme,
    Button,
    Pagination
} from "@carbon/react";
import { User, Search, Menu, Add, Download } from "@carbon/icons-react";
import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import "@carbon/charts/styles.css";
import "./Jobs.css";

const jobTableHeaders = [
    { key: "id", header: "Job ID" },
    { key: "name", header: "Name" },
    { key: "username", header: "User" },
    { key: "nnodes", header: "Nodes" },
    { key: "ntasks", header: "Tasks" },
    { key: "ncores", header: "Cores" },
    { key: "nodelist", header: "Node List" },
    { key: "t_submit", header: "Submit Time" },
    { key: "runtime", header: "Runtime (s)" },
    { key: "state", header: "State" },
    { key: "status", header: "Status" },
    { key: "actions", header: "" },
];

const Jobs = () => {
    const [isNavOpen, setIsNavOpen] = useState(true);
    const [jobData, setJobData] = useState([]);
    const [currentPage, setCurrentPage] = useState(1);
    const rowsPerPage = 7;
    const navigate = useNavigate();

    useEffect(() => {
        const fetchJobs = async () => {
            try {
                const res = await fetch("http://10.1.8.28:8080/flux/jobs");
                const data = await res.json();

                if (!data || !Array.isArray(data.jobs)) {
                    console.error("Invalid job data:", data);
                    return;
                }

                const jobs = data.jobs.map(job => ({
                    id: job.id.toString(),
                    name: job.name,
                    username: job.username,
                    nnodes: job.nnodes,
                    ntasks: job.ntasks,
                    ncores: job.ncores,
                    nodelist: job.nodelist ? job.nodelist : 'None',
                    t_submit: formatTimestamp(job.t_submit),
                    runtime: job.runtime ? job.runtime.toFixed(2) : '0',
                    state: job.state,
                    status: job.status,
                }));

                setJobData(jobs);
            } catch (error) {
                console.error("Error fetching jobs:", error);
            }
        };

        fetchJobs();
    }, []);

    const formatTimestamp = (timestamp) => {
        const date = new Date(timestamp * 1000);
        return date.toLocaleString();
    };

    const paginatedData = jobData.slice(
        (currentPage - 1) * rowsPerPage,
        currentPage * rowsPerPage
    );

    const handleCancelJob = async (jobId) => {
        // Xác nhận từ người dùng trước khi hủy
        const confirmCancel = window.confirm(`Are you sure you want to cancel job ${jobId}?`);
        if (!confirmCancel) return;

        const url = `http://10.1.8.28:8080/flux/jobs/${jobId}/cancel`;

        try {
            const res = await fetch(url, {
            method: "PUT",
            });

            if (!res.ok) {
            const errorText = await res.text();
            throw new Error(errorText || "Failed to cancel job");
            }

            alert(`Job ${jobId} canceled successfully.`);
            window.location.reload(); // Hoặc gọi lại fetchJobs() nếu bạn dùng useEffect
        } catch (err) {
            console.error("Error canceling job:", err);
            alert(`Failed to cancel job ${jobId}: ${err.message}`);
        }
        };


    const handleDownloadOutput = async (jobId) => {
        const url = `http://10.1.8.28:8080/flux/jobs/${jobId}/output?download=true`;

        try {
            const response = await fetch(url);

            if (!response.ok) {
                throw new Error(`Failed to fetch job output: ${response.statusText}`);
            }

            const blob = await response.blob();

            const downloadUrl = window.URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = downloadUrl;

            const disposition = response.headers.get("Content-Disposition");
            const match = disposition && disposition.match(/filename="?(.+)"?/);
            const filename = match ? match[1] : `job_${jobId}_output.zip`;

            a.download = filename;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            window.URL.revokeObjectURL(downloadUrl);
        } catch (err) {
            console.error("Download error:", err);
            alert(`Failed to download output for job ${jobId}`);
        }
    };

    return (
        <Theme theme="g10">
            <div className="dashboard-container">
                {/* Sidebar */}
                <div className={`custom-sidenav ${isNavOpen ? "open" : "closed"}`}>
                    <div className="sidenav-header">
                        {isNavOpen && <span className="sidenav-title">HPC-QC ResMan</span>}
                        <button className="menu-toggle" onClick={() => setIsNavOpen(!isNavOpen)}>
                            <Menu size={20} />
                        </button>
                    </div>
                    <ul>
                        <li><button className="sidenav-item logout-btn" onClick={()=>navigate("/admin/dashboard")}>🏠 Dashboard</button></li>
                        <li><button className="sidenav-item logout-btn" onClick={()=>navigate("/admin/fluxNode")}>📊 Flux Node</button></li>
                        <li><a href="#" className="sidenav-item active">📋 Jobs</a></li>
                        <li><button className="sidenav-item logout-btn" onClick={() => navigate("/admin/fileManage")}>📂 File</button></li>
                        <li><button className="sidenav-item logout-btn" onClick={()=>navigate("/")}>🚪 Logout</button></li>
                    </ul>
                </div>

                {/* Main Content */}
                <div className="main-content">
                    <div className="custom-header">
                        <div className="search-container">
                            <Search className="search-icon" />
                            <input type="text" placeholder="Search" className="search-input" />
                        </div>
                        <div className="user-section">
                            <User className="user-icon" />
                            <span>Admin</span>
                        </div>
                    </div>

                    <div className="content-wrapper">
                        <h2>Jobs</h2>

                        <div className="workload-section">
                            <div className="workload-header">
                                <h3>Workload</h3>
                                <Button className="create-job-btn" onClick={() => navigate("/admin/Createjob")} renderIcon={Add}>
                                    Create job
                                </Button>
                            </div>
                            <DataTable
                                rows={paginatedData}
                                headers={jobTableHeaders}
                                render={({ rows, headers, getHeaderProps }) => (
                                    <div className="table-container">
                                        <table className="bx--data-table">
                                        <thead>
                                            <tr>
                                            {headers.map((header) => (
                                                <th key={header.key} {...getHeaderProps({ header })}>
                                                {header.header}
                                                </th>
                                            ))}
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {rows.map((row) => {
                                                const jobId = row.cells.find(c => c.info.header === 'id').value;
                                                const jobState = row.cells.find(c => c.info.header === 'state').value;
                                            return (
                                                <tr key={row.id}>
                                                    {row.cells.map((cell) => (
                                                    <td key={cell.id}>{cell.value}</td>
                                                    ))}
                                                    <td>
                                                        {jobState === "ACTIVE" && (
                                                            <Button size="sm" className="create-job-btn" onClick={() => handleCancelJob(jobId)}>
                                                            Cancel
                                                            </Button>
                                                        )}

                                                        {row.cells.find(c => c.info.header === 'status')?.value === "COMPLETED" && (
                                                            <Button
                                                            size="sm"
                                                            className="download"
                                                            renderIcon={Download}
                                                            onClick={() => handleDownloadOutput(jobId)}
                                                            >
                                                            Download
                                                            </Button>
                                                        )}
                                                    </td>
                                                </tr>
                                            )})}
                                        </tbody>
                                        </table>
                                        <div className="horizontal-pagination">
                                            <Pagination
                                                backwardText=""
                                                forwardText=""
                                                page={currentPage}
                                                pageSize={rowsPerPage}
                                                pageSizes={[rowsPerPage]}
                                                totalItems={jobData.length}
                                                onChange={({ page }) => setCurrentPage(page)}
                                            />
                                        </div>
                                    </div>
                                )}
                                />
                        </div>
                    </div>
                </div>
            </div>
        </Theme>
    );
};

export default Jobs;
